...
export default function CheckoutPage(){
 ...
 useEffect(()=>{try{setCart(JSON.parse(localStorage.getItem("ptmarket-cart")||"[]"))}catch{};supabaseBrowser.auth.getUser().then(({data})=>{const u=data.user;if(u){const{data:p}=await supabaseBrowser.from("profiles").select("full_name,phone").eq("id",u.id).maybeSingle();setName(p?.full_name||u.user_metadata?.full_name||"");setPhone(p?.phone||u.user_metadata?.phone||"")}})},[]);
