...
export default function Header(){
  ...
  return <header className="sticky top-0 z-30 border-b bg-white/95 backdrop-blur">
    ...
    <a href="https://zalo.me/0353109444" target="_blank" rel="noreferrer" aria-label="Liên hệ chủ web qua Zalo" title="Liên hệ chủ web qua Zalo" className="fixed bottom-36 left-4 z-40 grid h-12 w-12 place-items-center rounded-full bg-[#0068ff] text-xl font-black text-white shadow-lg md:bottom-24 md:left-6">Z</a><Link href="/gio-hang" aria-label={"Giỏ hàng, "+cartCount+" sản phẩm"} className="fixed bottom-20 left-4 z-40 grid h-14 w-14 place-items-center rounded-full bg-brand-600 text-white shadow-lg md:bottom-6 md:left-6"><ShoppingCart size={24}/>{cartCount>0&&<span className="absolute -right-1 -top-1 min-w-6 rounded-full bg-red-500 px-1.5 py-1 text-center text-xs font-black text-white">{cartCount}</span>}</Link>
