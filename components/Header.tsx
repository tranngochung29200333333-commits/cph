...
function MessageIcon({unread=false}:{unread?:boolean}){return <span className="relative inline-flex"><MessageCircle size={19}/>{unread&&<span className="absolute -right-2 -top-2 min-w-4 rounded-full bg-red-500 px-1 text-center text-[9px] font-black leading-4 text-white">!</span>}</span>}

export default function Header(){
 const[unread,setUnread]=useState(false);
 useEffect(()=>{
   let active=true;
   const check=async()=>{const{data:{user}}=await supabaseBrowser.auth.getUser();if(!user){if(active)setUnread(false);return}const{count}=await supabaseBrowser.from("messages").select("id",{count:"exact",head:true}).eq("receiver_id",user.id).is("read_at",null);if(active)setUnread((count||0)>0)};
